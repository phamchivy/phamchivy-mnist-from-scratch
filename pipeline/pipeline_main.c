...
void pipeline_stage1_main(int forward_port, int backward_port) {
    ...
    NetworkStage* stage1 = create_stage(1, 784, 300, 0.01);
    if (!stage1) {
        ...
        for (int i = 0; i < number_imgs; i += current_mini_batch_size) {
            ...
            if (fwd_msg) {
                ...
                if (bwd_msg) {
                    ...
                    Matrix* gradients_from_stage2 = matrix_create(300, batch_size); // 300 hidden units
...
void pipeline_stage2_main(int forward_port, int backward_port, const char* stage1_ip) {
    ...
    NetworkStage* stage2 = create_stage(2, 300, 10, 0.01);
    if (!stage2) {
    ...
    while (training_active) {
        ...
        Matrix* activations = matrix_create(300, fwd_msg->label_count); // 300 hidden units
        int idx = 0;
